      ...
      {cards.map((card, idx) => (
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 1 }}>
            <Box sx={{ bgcolor: card.color, borderRadius: '50%', width: 18, height: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', mr: 1 }}>
              {card.icon}
            </Box>
            <Typography variant="subtitle1" sx={{ color: card.color, fontWeight: 600, fontSize: 17 }}>{card.label}</Typography>
          </Box>
          <Typography variant="h4" sx={{ fontWeight: 700, color: '#222', mb: 1, fontSize: 32 }}>{card.value}</Typography>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, position: 'absolute', bottom: 18, right: 18 }}>
            <TrendingUpIcon sx={{ fontSize: 18, color: card.color }} />
            <Typography variant="body2" sx={{ color: card.color, fontWeight: 500, fontSize: 14 }}>{card.growth}</Typography>
          </Box>
        </Paper>
      ))}
    </Box>
