      ...
      {cards.map((card, idx) => (
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 0.5 }}>
            <Box sx={{ bgcolor: card.color, borderRadius: '50%', width: 36, height: 36, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 1px 4px rgba(108,99,255,0.10)' }}>
              {card.icon}
            </Box>
            <Typography variant="subtitle1" sx={{ color: card.color, fontWeight: 600, fontSize: 16 }}>{card.label}</Typography>
          </Box>
          <Typography variant="h4" sx={{ fontWeight: 700, color: '#222', mb: 0.5, fontSize: 28 }}>{card.value}</Typography>
